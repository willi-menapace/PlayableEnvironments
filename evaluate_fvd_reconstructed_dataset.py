...
if __name__ == "__main__":
    ...
    evaluator_inferred_actions = getattr(importlib.import_module(config["evaluation"]["dataset_fvd_reconstruction_evaluator"]), 'evaluator')(config, logger, reference_datasets["test"], generated_datasets["test"])

    evaluation_results = evaluator_inferred_actions.compute_metrics()

    output_path = os.path.join(config["logging"]["output_directory"], "reconstructed_dataset_fvd_evaluation.yaml")
