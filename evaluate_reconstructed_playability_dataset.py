...
if __name__ == "__main__":
    ...
    reference_dataset_splits = DatasetSplitter.generate_evaluate_reconstructed_playability_dataset_splits(config, "reference")
    generated_dataset_splits = DatasetSplitter.generate_evaluate_reconstructed_playability_dataset_splits(config, "generated")
    ...
    evaluator = getattr(importlib.import_module(config["playable_model_evaluation"]["dataset_reconstruction_evaluator"]), 'evaluator')(config, logger, reference_datasets["test"], generated_datasets["test"])

    evaluation_results = evaluator.compute_metrics()

    output_path = os.path.join(config["logging"]["output_directory"], "reconstructed_playability_dataset_evaluation.yaml")
