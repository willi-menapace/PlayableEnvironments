...
class VideoSaver:
    ...
    @staticmethod
    def draw_text_on_frame(frame: np.ndarray, point: Tuple[int], text: str, fillcolor="white", shadowcolor="black", pointsize=30):
        '''
    ...
    def write_actions_on_frames(self, frames: np.ndarray, actions: List[int]) -> np.ndarray:
        ...
        for frame_idx in range(1, frames_count):
            ...
            current_frame = VideoSaver.draw_text_on_frame(current_frame, (10, 10), str(current_action), pointsize=16)
    ...
    def save_torch_video(self, frames: torch.Tensor, filename: str, framerate=30):
        '''
    ...
    def video_to_gif(self, video_filename: str, gif_filename: str, framerate=10, width=512):
        '''
        Converts a numpy array to videos
        ...
        command = ["ffmpeg",
                   ...
                   '-i',
                   video_filename,
                   '-vf', f'fps={framerate},scale={width}:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse',
                   '-loop', '0',
                   gif_filename]
