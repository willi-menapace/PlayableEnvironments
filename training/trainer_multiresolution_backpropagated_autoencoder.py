...
class TrainerMultiresolutionBackpropagatedAutoencoder(TrainerBackpropagatedAutoencoder):
    '''
    Helper class for model training
    '''

    def __init__(self, config, model, dataset: MulticameraVideoDataset, logger: Logger):
        super(TrainerMultiresolutionBackpropagatedAutoencoder, self).__init__(config, model, dataset, logger)
    ...
    def check_parameters(self):
        ...
        super(TrainerMultiresolutionBackpropagatedAutoencoder, self).check_parameters()
    ...
    def compute_losses(self, model, batch: Batch) -> Tuple:
        ...
        for result_type in results.keys():
            ...
            # The sampled positions corresponding to the highest resolution must be used to compute the crop.
            if self.crop_to_patch:
                reconstructed_observations = RayHelper.sample_original_region_from_patch_samples(reconstructed_observations, sampled_positions[0], self.autoencoder_downsample_factor[0])
            ...
            # Computes the reconstructon loss on autoencoder features at each downsampling factor
            total_autoencoder_features_reconstruction_loss = 0.0
            for downsample_factor_idx, (current_sampled_encoded_observations, current_reconstructed_encoded_observations) in enumerate(zip(sampled_encoded_observations, reconstructed_encoded_observations)):
                autoencoder_features_reconstruction_loss = self.autoencoder_reconstruction_loss(current_sampled_encoded_observations, current_reconstructed_encoded_observations)
                total_autoencoder_features_reconstruction_loss += autoencoder_features_reconstruction_loss.item()
                loss_info[f"{result_type}_autoencoder_features_reconstruction_loss_{downsample_factor_idx}"] = autoencoder_features_reconstruction_loss.item()
            ...
            loss_info[f"{result_type}_autoencoder_features_reconstruction_loss"] = total_autoencoder_features_reconstruction_loss
        ...
        for downsample_factor_idx in range(len(encoded_observations)):
            current_encoded_observations_mean = encoded_observations[downsample_factor_idx]  # The encoded observations are the mean

            if self.is_variational:
                current_encoded_observations_log_variance = results["encoded_observations_log_var"][downsample_factor_idx]
                stacked_distribution_parameters = torch.cat([current_encoded_observations_mean, current_encoded_observations_log_variance], dim=-3)
                kl_divergence_loss = self.kl_divergence_loss(stacked_distribution_parameters)
                loss_info[f"KL_loss_{downsample_factor_idx}"] = kl_divergence_loss.item()
                total_loss += self.config["training"]["loss_weights"]["KL_loss_lambda"] * kl_divergence_loss

                # Computes magnitude of the variance
                encoded_observations_variance_magnitude = torch.abs(torch.exp(current_encoded_observations_log_variance)).mean()
                loss_info[f"encoded_observations_variance_magnitude_{downsample_factor_idx}"] = encoded_observations_variance_magnitude.item()
...
def trainer(config, model, dataset, logger):
    return TrainerMultiresolutionBackpropagatedAutoencoder(config, model, dataset, logger)
