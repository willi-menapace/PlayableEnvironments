...
from training.losses import AutoencoderReconstructionLoss, ImageReconstructionLoss, \
    SquaredL2NormLoss, SpatialKLGaussianDivergenceLoss
from training.trainer import Trainer
from utils.lib_3d.ray_helper import RayHelper
from utils.logger import Logger


class TrainerBackpropagatedAutoencoder(Trainer):
    '''
    Helper class for model training
    '''

    def __init__(self, config, model, dataset: MulticameraVideoDataset, logger: Logger):
        super(TrainerBackpropagatedAutoencoder, self).__init__(config, model, dataset, logger)

        autoencoder_reconstruction_loss_type = config["training"]["loss_weights"]["autoencoder_features_reconstruction_loss_type"]
        autoencoder_reconstruction_loss_normalize = config["training"]["loss_weights"]["autoencoder_features_reconstruction_loss_normalize"]
        self.autoencoder_reconstruction_loss = AutoencoderReconstructionLoss(type=autoencoder_reconstruction_loss_type, normalize=autoencoder_reconstruction_loss_normalize)

        # Size of the feature map to reconsturct to use as input to the decoder
        # If 0 features are sampled randomly
        self.patch_size = config["training"]["patch_size"]
        self.align_grid = config["training"]["align_grid"]
        self.crop_to_patch = config["training"]["crop_to_patch"]
        # Checks the validity of configuration parameters
        self.check_parameters()
        self.autoencoder_downsample_factor = config["model"]["autoencoder"]["downsample_factor"]

        # Initializes losses
        self.reconstruction_loss = ImageReconstructionLoss(use_radial_weights=self.use_radial_weights)
        ...
        self.frozen_autoencoder_steps = config["training"]["frozen_autoencoder_steps"]

        self.is_variational = False
        if "variational" in model.autoencoder_model.model_config and model.autoencoder_model.model_config["variational"]:
            self.is_variational = True

    def check_parameters(self):
        '''
        Performs checks for the initial parameters to find discrepancies in configuration early
        Raises an Exception if the checks are not satisfied
        :return:
        '''
    ...
    def get_optimizer(self, model):
        ...
        autoencoder_parameters = model.get_autoencoder_parameters()
        main_parameters = model.get_main_parameters()
        object_encoder_parameters = model.get_object_encoder_parameters()
        camera_offsets_parameters = model.get_camera_offsets_parameters()

        main_learning_rate = self.config["training"]["learning_rate"]
        main_weight_decay = self.config["training"]["weight_decay"]
        autoencoder_learning_rate = main_learning_rate

        if "autoencoder_learning_rate" in self.config["training"]:
            autoencoder_learning_rate = self.config["training"]["autoencoder_learning_rate"]
        else:
            raise Exception("Autoencoder learning rate was not set explicitly, is this what you intended?")

        main_param_group = {"params": main_parameters}
        object_encoder_group = {"params": object_encoder_parameters}
        camera_offsets_param_group = {"params": camera_offsets_parameters}
        autoencoder_param_group = {
            "params": autoencoder_parameters,
            "lr": autoencoder_learning_rate
        }
        optimizer = torch.optim.Adam([main_param_group, object_encoder_group, camera_offsets_param_group, autoencoder_param_group],
                                          lr=main_learning_rate,
                                          weight_decay=main_weight_decay)

        return optimizer

    def save_reconstructed_observation(self, reconstructed_observation: torch.Tensor):
        '''
        Saves the given reconstructed observation
        :param reconstructed_observation: (3, height, width) reconstructed observation to save
        :return:
        '''

        output_directory = os.path.join(self.config["logging"]["output_images_directory"], f"training_reconstructed_observation_{self.global_step:05d}")
        Path(output_directory).mkdir(parents=True, exist_ok=True)

        output_image_path = os.path.join(output_directory, f"reconstructed_observation.png")
        save_image(reconstructed_observation, output_image_path)
    ...
    def compute_losses(self, model, batch: Batch) -> Tuple:
        ...
        sampled_encoded_observations = results["sampled_encoded_observations"]
        sampled_positions = results["positions"]

        # The complete encoded observations
        ...
        for result_type in results.keys():
            ...
            reconstructed_observations = results[result_type]["global"]["reconstructed_observations"]
            ...
            if self.global_step % self.config["training"]["image_save_interval"] == 0:
                self.save_reconstructed_observation(reconstructed_observations[0, 0, 0])

            # If cropping is to be used, crop the reconstructed results before the loss
            if self.crop_to_patch:
                reconstructed_observations = RayHelper.sample_original_region_from_patch_samples(reconstructed_observations, sampled_positions, self.autoencoder_downsample_factor)
            ...
            # Computes the reconstructon loss on autoencoder features
            autoencoder_features_reconstruction_loss = self.autoencoder_reconstruction_loss(sampled_encoded_observations, reconstructed_encoded_observations)
            ...
            loss_info[f"{result_type}_autoencoder_features_reconstruction_loss"] = autoencoder_features_reconstruction_loss.item()
            ...
            total_loss += self.config["training"]["loss_weights"]["autoencoder_features_reconstruction_loss_lambda"] * autoencoder_features_reconstruction_loss
        ...
        # Computes the KL loss if the autoencoder is variational
        if self.is_variational:
            encoded_observations_mean = encoded_observations # The encoded observations are the mean if the autencoder is variational
            encoded_observations_log_variance = results["encoded_observations_log_var"]
            stacked_distribution_parameters = torch.cat([encoded_observations_mean, encoded_observations_log_variance], dim=-3)
            kl_divergence_loss = self.kl_divergence_loss(stacked_distribution_parameters)
            loss_info[f"KL_loss_lambda"] = kl_divergence_loss.item()
        ...
        with torch.no_grad():
            ...
            encoded_observations_l2_norm = torch.sqrt(encoded_observations.pow(2).sum(-3)).mean()
            loss_info[f"encoded_observations_l2_norm"] = encoded_observations_l2_norm.item()
            if self.is_variational:
                encoded_observations_variance_magnitude = torch.abs(torch.exp(encoded_observations_log_variance)).mean()
                loss_info[f"encoded_observations_variance_magnitude"] = encoded_observations_variance_magnitude.item()
...
def trainer(config, model, dataset, logger):
    return TrainerBackpropagatedAutoencoder(config, model, dataset, logger)
