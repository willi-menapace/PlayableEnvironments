...
from training.losses import AutoencoderReconstructionLoss
...
class TrainerAutoencoder(Trainer):
    ...
    def __init__(self, config, model, dataset: MulticameraVideoDataset, logger: Logger):
        super(TrainerAutoencoder, self).__init__(config, model, dataset, logger)
    ...
    def compute_losses(self, model, batch: Batch) -> Tuple:
        ...
        results = model(observations, camera_rotations, camera_translations, focals, bounding_boxes, bounding_boxes_validity, global_frame_indexes, video_frame_indexes, video_indexes, samples_per_image=self.samples_per_image,
                        perturb=self.perturb, shuffle_style=self.shuffle_style, patch_size=self.patch_size, patch_stride=self.autoencoder_downsample_factor)
        ...
        for result_type in results.keys():
            ...
            reconstruction_loss = self.autoencoder_reconstruction_loss(sampled_encoded_observations, reconstructed_encoded_observations)
            ...
            loss_info[f"{result_type}_autoencoder_features_reconstruction_loss"] = reconstruction_loss.item()
            ...
            total_loss += self.config["training"]["loss_weights"]["autoencoder_features_reconstruction_loss_lambda"] * reconstruction_loss
...
def trainer(config, model, dataset, logger):
    return TrainerAutoencoder(config, model, dataset, logger)
