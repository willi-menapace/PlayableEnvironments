...
if __name__ == "__main__":
    ...
    dataset_splits = DatasetSplitter.generate_dataset_reconstruction_splits(config)
    ...
    trainer = getattr(importlib.import_module(config["training"]["trainer"]), 'trainer')(config, model, datasets["validation"], logger)
    ...
    validation_dataset_creator = getattr(importlib.import_module(config["evaluation"]["dataset_creator"]), 'dataset_creator')(config, datasets["validation"], logger, name="val")
    test_dataset_creator = getattr(importlib.import_module(config["evaluation"]["dataset_creator"]), 'dataset_creator')(config, datasets["test"], logger, name="test")
    ...
    #validation_dataset_creator.reconstruct_dataset(model)
