    ...
    def __init__(self, model_config: Dict):
        ...
        for downsampling_layer_set_idx, current_downsampling_layers_count in enumerate(self.downsampling_layers_count):
            ...
            for i in range(current_downsampling_layers_count):  # add downsampling layers
                ...
                # If too much downsampling layers were used, add residual blocks after the first 2 downsamplings
                if current_downsampling_layers_count >= 3 and i == 1:
                    # Adds the bottleneck layers
                    for bottleneck_idx in range(self.bottleneck_blocks):  # add ResNet blocks

                        out_dim = initial_features_count * mult * 2

                        layer_list.append(
                                            CycleGanResnetBlock(out_dim, padding_type=padding_type,
                                                norm_layer=norm_layer,
                                                use_dropout=use_dropout,
                                                use_bias=use_bias, out_dim=out_dim))

                        # Need to add an activation after the residual
            ...
            for bottleneck_idx in range(self.bottleneck_blocks):  # add ResNet blocks
                ...
                # Need to add activation after the residual block unless this is the last residual
                if bottleneck_idx != self.bottleneck_blocks - 1:
                    layer_list.append(nn.ReLU(True))
