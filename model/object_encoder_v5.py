...
class ObjectEncoderV5(nn.Module):
    '''
    ...
    def __init__(self, config: Dict, model_config: Dict):
        ...
        '''
        super(ObjectEncoderV5, self).__init__()
        ...
        self.style_head = nn.Linear(512, self.style_features)
        self.deformation_head = nn.Linear(512, self.deformation_features)
    ...
    def forward(self, observations: torch.Tensor, bounding_boxes: torch.Tensor, camera_rotations: torch.Tensor, camera_translations: torch.Tensor,
        ...
        # Creates a fake attention
        attention = torch.zeros_like(initial_output[:, -1:])
...
def model(config, model_config):
    ...
    :return:
    '''
    return ObjectEncoderV5(config, model_config)
